...
const ContactTextPartWithImg = () => {
  const itemTextPartWithImg = itemsTextPartWithImg.map((item, index) => {
    ...
    return (
            <StyledBoxOneIdea key={item.title}
                              isEven={isEven}
            >
  ...
  return (
        ...
        >
            <TitleAndText variant={{ _: 'title3Uppercase', tablet: 'title2Uppercase' }} textAlign={'center'} color={'colorPrimary'} mb={20}>
                Etiam commodo ac arcu nec porta
            </TitleAndText>
            <TitleAndText variant={{ _: 'textSmall', tablet: 'textMiddleBold' }} textAlign={'center'} color={'colorPrimary'} mb={20}>
