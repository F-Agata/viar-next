...
const BoxSingleQuestion = ({ question, smallDescription, opened = false }) => {
  ...
  return (
        <StyledBoxSingleQuestion>
            ...
            >
                <Box pr={10}>
                    {!isOpen && <Box onClick={() => setIsOpen(!isOpen)}
                                     width={36}
                                     height={36}
                                     bg={'colorWhite'}
                                     display={'flex'}
                    ...
                    {isOpen && <BoxIcon onClick={() => setIsOpen(!isOpen)}
                                        width={36}
                                        height={36}
                                        bg={'colorSecondary'}
                                        display={'flex'}
                ...
                <TitleAndText variant={'textMiddleBold'} textAlign={'left'} >{question}</TitleAndText>

            </Box>
            {isOpen && <TitleAndText variant={'textSmall'} textAlign={'left'} p={'20px 0px'}>{smallDescription}</TitleAndText>}
        </StyledBoxSingleQuestion>
  );
};
